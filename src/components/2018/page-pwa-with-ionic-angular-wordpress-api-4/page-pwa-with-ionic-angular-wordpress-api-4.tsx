...
return this.page < this.totalPages;
                            ...
                            <ul>
                                <li>Line 17 – We added a class variable called <code>page</code> to track the current or last page (set of posts) that we’ve loaded.</li>
                                <li>Line 18 – We added a class variable called <code>totalPages</code> to hold the total number of pages of posts that are available. This is needed so that we can disable the infinite scroll component when we’ve reached the last of the available pages. If we don’t disable the component, it will try to fetch a page number that isn’t available and thus result in an error.</li>
                                <li>Line 31 – In the <code>getPosts()</code> function, we modified the HttpClient’s&nbsp;<code>get</code> call. We added&nbsp;<code>&#123;observe: 'response'&#125;</code> to specify that we want the whole <code>HttpResponse</code> back, rather than just the body. This is necessary because we need to inspect the response to find the&nbsp;<code>X-WP-TotalPages</code> header, which WordPress gives to tell us how many&nbsp; total pages are available.</li>
                                <li>Line 39 – We added a <code>getMorePosts()</code> function that increments&nbsp; the current page variable, gets the next page of posts, and then pushes them onto the array of posts that we already have.</li>
                                <li>Line 47 – We modified the&nbsp;<code>processPostData()</code> function to read the&nbsp;<code>X-WP-TotalPages</code> header and set the&nbsp;<code>totalPages</code> class variable.</li>
                                <li>Line 58 – Finally, we added a&nbsp;<code>hasMorePosts()</code> convenience function to return <code>true</code> or <code>false</code> depending on whether more pages are available to be loaded.</li>
                            </ul>
                            ...
                            <pre><code class="language-ts">{`import {Component, OnInit, ViewChild, ViewEncapsulation} from '@angular/core';
...
return true;
                            ...
                            <ul>
                                ...
                                <li>Line 29 – A new <code>getMorePosts()</code> function pretty much just proxies to the function of the same name that’s in the <code>DataService</code>. I uses&nbsp;<code>this.infiniteScroll.complete()</code> to remove the loading&nbsp; spinner when it’s done doing its thing.</li>
                                <li>Line 35 – The new&nbsp;<code>infiniteScrollDisabled()</code> function exposes a <code>true/false</code> property that will be bound to the <code>ion-infinite-scroll</code> component in the view. This will be used&nbsp; to ensure that the infinite scroll component gets disabled when there are no more post pages available to load.</li>
                            </ul>
                            ...
                            <pre><code class="language-html">{`<ion-infinite-scroll threshold="100px" (ionInfinite)="getMorePosts($event)" [disabled]="infiniteScrollDisabled()">
<ion-infinite-scroll-content loadingSpinner="bubbles" loadingText="Loading more data...">
</ion-infinite-scroll-content>
</ion-infinite-scroll>`}</code></pre>
