...
@Component({
  tag: 'page-technology',
  styleUrl: 'page-technology.css'
})
export class PageTechnology {
  ...
  async componentWillLoad() {
    if (debug) {
      console.log('> PageTechnology.componentWillLoad');
    }
    ...
    let excludeTracks = await get(EXCLUDE_TRACKS)
      ...
      .catch(function (err) {
        if (debug) {
          console.log('< PageTechnology.componentWillLoad < No exclude tracks saved; returning empty array. [%o]', err);
        }
        // do nothing
        return [];
      });
  ...
  modalDidDismiss(event: CustomEvent) {
    if(debug){
      console.log('> PageTechnology.modalDidDismiss > event.detail.data: %o',event.detail.data);
    }
  ...
  async updateContentList() {
    ...
    if (debug) {
      console.log('> PageTechnology.updateContentList');
    }
  ...
  async presentFilter() {
    if (debug) {
      console.log('>> PageTechnology.presentFilter');
    }
