...
public boolean render(final RenderingPluginModel p_model) throws RenderingPluginException {
    ...
    try {
        ...
        if (!params.containsKey(CMPNT_NAME)) {
            writer.write(" <span style="color: red">WCM Velocity plugin parameter '" + CMPNT_NAME + "' is required, but has either been omitted or mispelled.</span> ");
        }
        if (!params.containsKey(LIBRARY_NAME)) {
            writer.write(" <span style="color: red">WCM Velocity plugin parameter '" + LIBRARY_NAME + "' is required, but has either been omitted or mispelled. ");
    ...
    return renderBody;
                            ...
                            <ul>
                                ...
                                <li>Line 32: To work with the WCM API, we must get a handle on the workspace. The workspace is the main interface to everything in WCM using the WCM API. In this case, we’re using the system workspace with full access to everything in WCM. The following line is commented out, but it shows you how you could get a workspace by logging in as the current user if you wanted to.</li>
                                <li>Lines 35-38: We attempt to set the current Document Library used by the workspace, which can hopefully help us avoid errors and make our operations more efficient.</li>
                                <li>Line 40: Just about everything in the WCM API extends from the Document class. This confused me when I first started working with the WCM API because I thought that Document meant <em>content item</em> (as opposed to a component, for example). That’s not the case. Document is simply the superclass for just about every model object you work with in the API. So anyway – this line searches for the given HTML component by name and returns an iterator of DocumentId objects.</li>
                                <li>Line 44: We only expect one component to be returned, so we only have to worry about dealing with the first in the iterator.</li>
                                <li>Line 46: And we expect it to be a LibraryHTMLComponent, so we cast it as such.</li>
                                <li>Line 47: We render the HTML component to get its contents, which will be the raw Velocity template (before being merged with objects in the Velocity context). You could use the API to just get the contents of the HTML element rather than rendering it, but if you did that, none of the WCM tags and references that might exist in the component would be properly rendered. You’d just get the raw source. In this case, we want WCM to render the component and then the results of that will be the raw Velocity template.</li>
                                <li>For the rest, we’re essentially doing the same thing that we did in Part 1 of this tutorial series. We are creating a Velocity context, shoving some objects into it, and then merging the context and the template in a rendering. We’re also parsing the value of the <em>variables </em>attribute if the user provides variables to the tag and then were injecting those into the Velocity context. This is the place where you could continue to evolve the plugin and put your own interesting objects into the context if you wanted to. My idea is that the plugin should allow users to define variables, but then also supply some interesting objects from Portal and WCM. That’s an exercise for a later day, perhaps.</li>
                            </ul>
